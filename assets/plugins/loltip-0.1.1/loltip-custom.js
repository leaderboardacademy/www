...
jQuery(document).ready(function () {
    setTimeout(function () {
        ...
        $(".loltip-placeholder").each(function () {
            ++placeholderCounter;

            var containerId = "loltip_placeholder_" + placeholderCounter;
            var subSubElement = $('<div />', {
                        'class': 'loltip-content',
                        'id': containerId,
                        'aria-atomic': true
                });
            var subElement = $('<div />', { 'class': 'loltip loltip-default loltip-tipsy' });
            subElement.html(subSubElement);
            $(this).html(subElement);

            var href = $(this).attr('data-loltip-href');

            var respDom = $("#" + containerId);
            if (1 === respDom.length) {
                respDom.html(LoLTip.Templates.loading);
            } else {
                return false;
            }
            if (!href || href == 'undefined') {
                respDom.html('<p>Entity not found.</p>');
                return false;
            }
            ...
            jQuery.ajax({
                ...
                url: href + '/json',
                dataType: 'jsonp',
                jsonpCallback: containerId
            })
            .fail(function (jqXHR, textStatus, errorThrown) {
                var respSelector = "#" + this.jsonpCallback;
                var respDom = $(respSelector);
                if (1 === respDom.length) {
                    respDom.html(new t(LoLTip.Templates.failed).render({
                            text: textStatus,
                            error: errorThrown
                        }));
                }
